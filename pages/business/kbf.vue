<template>
	<view>
		<view class="contain">
			<u-navbar title="经营发展管理 — 新店开办费" leftText="返回" :fixed="true" @rightClick="rightClick" @leftClick="leftClick">
			</u-navbar>
			<view class="cont-block">
				...
				<view class="flex-data">
					<fourblock
					:leftStyle="{fontSize:'32rpx',color:'#FF4317'}"
					:rightStyle="{fontSize:'32rpx',color:'#1FCD27'}"
					:right2Style="{fontSize:'32rpx',color:'#1D97FF'}"
					:itemValue="['5,564.00元','2家','5,564.00元','86.55%']"
					:itemName="['近三个月新店开办费总额','近三个月新店总数','近三个月新店开办费预算','预算符合率']">
					</fourblock>
				</view>
				<view class="jxh-kb">
					<view class="title">
						<image src="../../static/image/circle.png"></image>
						<text>新店开办费金额及符合率</text>
					</view>
					<myprocess 
					:list="processList"
					></myprocess>
...
<script>
  ...
  import fourblock from "@/components/flex-data/fourblock.vue"

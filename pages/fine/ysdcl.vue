<template>
	<view>
		<view class="contain">
			<u-navbar title="营收预算达成率" leftText="返回" :fixed="true" @rightClick="rightClick" @leftClick="leftClick">
			</u-navbar>
			<view class="cont-block">
				...
				<view class="flex-data">
					<flexData :itemName="['营收预算达成率','周环比增长']" :itemValue="[sybjxhzbysdclist[0].ysdc + '%',sybjxhzbysdclist[0].ysdchb + '%']"
						:leftStyle="{fontSize:'32rpx',color:'#1D97FF'}"
						:rightStyle="{fontSize:'32rpx',color:'#FF4317'}"></flexData>
				</view>

				<view class="jxh-kb">
					<view class="title">
						<image src="../../static/image/circle.png"></image>
						<text>营收预算达成率（后10）</text>
					</view>
					<myprocess :list="processList"></myprocess>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			getData() {
				this.$helper.httpPost(this.$api.getYsdcData_post, {
					...
					.then((res) => {
						this.mdjxhzbysdclist = res.data.mdjxhzbysdclist
						this.sybjxhzbysdclist = res.data.sybjxhzbysdclist
						//天数最大值
						let _processList = []
						this.mdjxhzbysdclist.forEach((item,index) => {
							_processList[index] = {
								name: item.posname,
								percentage: (item.ysdc / this.mdjxhzbysdclist[0].ysdc).toFixed(2),
								accounted: item.ysdc + '%',

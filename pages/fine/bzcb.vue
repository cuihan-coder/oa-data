<template>
	<view>
		<view class="contain">
			<u-navbar title="包装成本" leftText="返回" :fixed="true" @rightClick="rightClick" @leftClick="leftClick">
			</u-navbar>
			<view class="cont-block">
				...
				<view class="flex-data">
					<flexData 
					:itemName="['包装成本占比','周环比增长']"
					:itemValue="[sybjxhzbbzcblist[0].bzcb +  '%',sybjxhzbbzcblist[0].bzcbhb + '%']"
...
import pickers from '@/components/picker/index.vue'
	export default {
		...
		data() {
			...
			methods: {
				getData() {
					this.$helper.httpPost(this.$api.getbzData_post, {
						...
						.then((res) => {
							this.mdjxhzbbzcblist = res.data.mdjxhzbbzcblist
							this.sybjxhzbbzcblist = res.data.sybjxhzbbzcblist
							
							//天数最大值
							let _processList = []
							this.mdjxhzbbzcblist.forEach((item,index) => {
								_processList[index] = {
									name: item.posname,
									percentage: (item.bzcb / this.mdjxhzbbzcblist[0].bzcb).toFixed(2),
									accounted: item.bzcb + '%',

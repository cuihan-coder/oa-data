<template>
	<view>
		<view class="contain">
			<u-navbar title="定额食品成本" leftText="返回" :fixed="true" @rightClick="rightClick" @leftClick="leftClick">
			</u-navbar>
			<view class="cont-block">
				...
				<view class="flex-data">
					<flexData 
					:itemName="['定额食品成本占比','周环比增长']"
					:itemValue="[sybjxhzbspcblist[0].spcb + '%',sybjxhzbspcblist[0].spcbhb + '%']"
...
import pickers from '@/components/picker/index.vue'
	export default {
		...
		data() {
			...
			methods: {
				getData() {
					this.$helper.httpPost(this.$api.getspcbData_post, {
						...
						.then((res) => {
							this.mdjxhzbspcblist = res.data.mdjxhzbspcblist
							this.sybjxhzbspcblist = res.data.sybjxhzbspcblist
							
							//天数最大值
							let _processList = []
							this.mdjxhzbspcblist.forEach((item,index) => {
								_processList[index] = {
									name: item.posname,
									percentage: (item.spcb / this.mdjxhzbspcblist[0].spcb).toFixed(2),
									accounted: item.spcb + '%',

<template>
	<view>
		<view class="contain">
			<u-navbar title="精细化管理 - 员工餐" leftText="返回" :fixed="true" @rightClick="rightClick" @leftClick="leftClick">
			</u-navbar>
			<view class="cont-block">
				...
				<view class="flex-data">
					<flexData 
					:itemName="['员工餐占比','周环比增长']"
					:itemValue="[sybjxhzbygclist[0].ygc + '%',sybjxhzbygclist[0].ygchb + '%']"
...
import pickers from '@/components/picker/index.vue'
	export default {
		...
		computed: {
			...
			methods: {
				getData() {
					this.$helper.httpPost(this.$api.getYgcData_post, {
						...
						.then((res) => {
							this.mdjxhzbygclist = res.data.mdjxhzbygclist
							this.sybjxhzbygclist = res.data.sybjxhzbygclist
							
							//天数最大值
							let _processList = []
							this.mdjxhzbygclist.forEach((item,index) => {
								_processList[index] = {
									name: item.posname,
									percentage: (item.ygc / this.mdjxhzbygclist[0].ygc).toFixed(2),
									accounted: item.ygc + '%',

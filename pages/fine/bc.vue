<template>
	<view>
		<view class="contain">
			<u-navbar title="报损" leftText="返回" :fixed="true" @rightClick="rightClick" @leftClick="leftClick">
			</u-navbar>
			<view class="cont-block">
				...
				<view class="flex-data">
					<flexData 
					:itemName="['报损占比','周环比增长']"
					:itemValue="[sybjxhzbygclist[0].bs + '%',sybjxhzbygclist[0].bshb + '%']"
...
import pickers from '@/components/picker/index.vue'
	export default {
		...
		computed: {
			...
			methods: {
				getData() {
					this.$helper.httpPost(this.$api.getbsData_post, {
						...
						.then((res) => {
							...
							this.mdjxhzbygclist.forEach((item,index) => {
								_processList[index] = {
									name: item.posname,
									percentage: (item.bs / this.mdjxhzbygclist[0].bs).toFixed(2),
									accounted: item.bs + '%',

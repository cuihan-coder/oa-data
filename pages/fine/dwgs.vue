<template>
	<view>
		<view class="contain">
			<u-navbar title="单位工时销售" leftText="返回" :fixed="true" @rightClick="rightClick" @leftClick="leftClick">
			</u-navbar>
			<view class="cont-block">
				...
				<view class="flex-data">
					<flexData 
					:itemName="['SMH','周环比增长']"
					:itemValue="[sybyqzkList[0].smh + '元/小时', sybyqzkList[0].smhhb + '%']"
...
import pickers from '@/components/picker/index.vue'
	export default {
		...
		computed: {
			...
			methods: {
				getData() {
					this.$helper.httpPost(this.$api.getSmhData_post, {
						...
						.then((res) => {
							...
							//天数最大值
							let _processList = []
							this.mdyqzkList.forEach((item,index) => {
								_processList[index] = {
									name: item.posname,
									percentage: (item.smh / this.mdyqzkList[0].smh).toFixed(2),
									accounted: item.smh + '元/时',

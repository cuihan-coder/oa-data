<template>
	<view>
		<view class="contain">
			<u-navbar title="人事成本" leftText="返回" :fixed="true" @rightClick="rightClick" @leftClick="leftClick">
			</u-navbar>
			<view class="cont-block">
				...
				<view class="flex-data">
					<flexData 
					:itemName="['人事成本占比','周环比增长']"
					:itemValue="[sybjxhzbrsflist[0].rsf + '%',sybjxhzbrsflist[0].rsfhb + '%']"
...
import pickers from '@/components/picker/index.vue'
	export default {
		...
		computed: {
			...
			methods: {
				getData() {
					this.$helper.httpPost(this.$api.getRsfzbData_post, {
						...
						.then((res) => {
							this.mdjxhzbrsflist = res.data.mdjxhzbrsflist
							this.sybjxhzbrsflist = res.data.sybjxhzbrsflist
							
							//天数最大值
							let _processList = []
							this.mdjxhzbrsflist.forEach((item,index) => {
								_processList[index] = {
									name: item.posname,
									percentage: (item.rsf / this.mdjxhzbrsflist[0].rsf).toFixed(2),
									accounted: item.rsf + '%',

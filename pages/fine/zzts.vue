<template>
	<view>
		<view class="contain">
			<u-navbar title="库存周转天数" leftText="返回" :fixed="true" @rightClick="rightClick" @leftClick="leftClick">
			</u-navbar>
			<view class="cont-block">
				...
				<view class="flex-data">
					<flexData 
					:itemName="['库存周转天数','周环比增长']"
					:itemValue="[sybjxhzbczztslist[0].kczzts + '天', sybjxhzbczztslist[0].kczztshb + '%']"
  ...
  import pickers from '@/components/picker/index.vue'
	export default {
		...
		data() {
			...
			methods: {
				getData() {
					this.$helper.httpPost(this.$api.getkczztsData_post, {
						...
						.then((res) => {
							this.mdjxhzbkczztslist = res.data.mdjxhzbkczztslist
							this.sybjxhzbczztslist = res.data.sybjxhzbczztslist
							
							//天数最大值
							let _processList = []
							this.mdjxhzbkczztslist.forEach((item,index) => {
								_processList[index] = {
									name: item.posname,
									percentage: (item.kczzts / this.mdjxhzbkczztslist[0].kczzts).toFixed(2),
									accounted: item.kczzts + '%',
